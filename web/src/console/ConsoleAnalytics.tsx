...
export function ConsoleAnalytics({ globalScope }: { globalScope: boolean }) {
  ...
  const modelUnattributedRequests = data?.summary.unattributed_requests || 0
  const flowUnattributedRequests = data?.summary.flow_unattributed_requests || 0
  ...
  const projectionDiffers = Boolean(
    modelUnattributedRequests || flowUnattributedRequests || modelUnattributedQuota || flowUnattributedQuota
  ...
  return <div className="console-page console-analytics-page">
    ...
    {loading && !data ? <ConsoleLoading /> : error && !data ? <ConsoleError message={error} retry={() => void load()} /> : data && <>
      ...
      <section className="console-metric-grid">
        <ConsoleMetric icon={<BarChart3 size={19} />} label={t('请求数')} value={numberText(data.summary.requests)} detail={modelUnattributedRequests ? t('{{count}} 条未进入当前模型归集明细', { count: modelUnattributedRequests }) : data.reconciliation.requests_exact ? t('实时日志总数') : t('小时归集请求数')} tone="blue" />
      ...
      <div className="analytics-data-contract"><Layers3 size={17} /><div><strong>{exactTotals ? t('数据口径已对齐') : t('精确总量暂不可用')}</strong><span>{!exactTotals ? t('上游未返回完整的实时日志总量，当前暂以小时归集数据展示；恢复后会自动切回精确总量。') : projectionDiffers ? t('请求数与额度以实时消费日志为准；小时归集明细可能因归集时差、时间边界、保留策略或缺少归因维度产生差额，差额已单独列出。') : t('请求数与额度来自实时消费日志；模型、Token 与额度流向来自 New API 小时归集，当前已完全闭合。')}</span></div><ConsoleBadge tone={!exactTotals || projectionDiffers ? 'amber' : 'green'}>{!exactTotals ? t('小时归集回退') : projectionDiffers ? t('存在归集差额') : t('归集已追平')}</ConsoleBadge></div>
      ...
      <section className="console-two-column console-analytics-detail-grid">
        <article className="console-panel">
          ...
          {modelRows.length || modelUnattributedQuota > 0 || modelUnattributedRequests > 0 ? <div className="console-ranking-list">{visibleModelRows.map((item, index) => <div key={item.model}><b>{String(index + 1).padStart(2, '0')}</b><span><strong>{item.model}</strong><small>{numberText(item.requests)} {t('次请求')} · {numberText(item.tokens)} Tokens</small></span><em>{quotaText(item.quota, data.quota_per_unit)}</em></div>)}
            ...
            {(modelUnattributedQuota > 0 || modelUnattributedRequests > 0) && <div className="analytics-pending-row"><b>··</b><span><strong>{t('总量与归集明细差额')}</strong><small>{modelUnattributedRequests ? t('{{count}} 条请求已计入实时总数，但未进入当前模型与 Token 归集明细', { count: modelUnattributedRequests }) : t('实时额度已计入总量，模型归集明细仍存在差额')}</small></span><em>{quotaText(modelUnattributedQuota, data.quota_per_unit)}</em></div>}
        ...
        <article className="console-panel">
          ...
          {flowRows.length || flowUnattributedQuota > 0 || flowUnattributedRequests > 0 ? <div className="console-flow-list">{flowRows.slice(0, 12).map((item) => <div key={item.key}><span><strong>{item.token_name || item.username || `#${item.token_id}`}</strong><small>{item.use_group || t('默认分组')} · {item.model_name}</small></span><em>{quotaText(item.quota, data.quota_per_unit)}<small>{numberText(item.count)} {t('次调用')}</small></em></div>)}
            ...
            {(flowUnattributedQuota > 0 || flowUnattributedRequests > 0) && <div className="analytics-pending-row"><span><strong>{t('总量与归集明细差额')}</strong><small>{flowUnattributedRequests ? t('{{count}} 条请求已计入实时总数，但未进入当前密钥与分组流向明细', { count: flowUnattributedRequests }) : t('实时额度已计入总量，密钥与分组流向明细仍存在差额')}</small></span><em>{quotaText(flowUnattributedQuota, data.quota_per_unit)}</em></div>}
