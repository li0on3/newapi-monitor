...
class NewAPISessionVerifier:
    def __init__(self, base_url: Callable[[], str], cache_seconds: int = 30, timeout_seconds: int = 8):
    ...
    def verify(self, session_cookie: str, user_id: str) -> dict[str, Any] | None:
        if not session_cookie or not user_id.isdigit() or int(user_id) <= 0:
            return None
        ...
        try:
            ...
            if isinstance(data, dict) and int(data.get("status") or 0) == 1:
                identity = {
                    "source": "newapi",
