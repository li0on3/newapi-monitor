...
Analytics uses real New API data to render a model-stacked timeline and can switch between Requests, Tokens, and Spend. Request and spend totals always use live consumption logs as the authoritative source; model, token, and usage-flow details come from New API's hourly `quota_data` attribution. When the two scopes differ, the page shows a signed total-to-attribution difference and explains that aggregation lag, time-bucket boundaries, retention policy, or missing historical dimensions such as group may be responsible. It does not promise that every positive difference is merely pending and will later disappear. Usage-flow rows are merged by the account/key, group, and model dimensions visible in the UI; hidden node and channel dimensions cannot create duplicate-looking rows, and everything after row 12 is summarized as Other before the reconciliation row closes the view to the authoritative total. A slower stale request cannot overwrite the user's later selection.
