...
| Analytics model/token/flow | `/api/data/`, `/api/data/flow`, and self variants | New API hourly attribution; differences caused by aggregation lag, time-bucket boundaries, retention scope, or missing historical dimensions such as group are shown separately | Regular-user ranges over 30 days are split to respect the upstream limit; any failed chunk fails the whole query |
...
A positive difference only means the live-log total is greater than hourly attribution; causes can include attribution lag, bucket boundaries, different retention windows, or older rows that lack model, key, or group attribution dimensions. A negative difference means hourly attribution is greater than currently queryable logs and can likewise come from retention or bucket boundaries. The platform does not infer one definitive cause from the sign alone and does not promise that every positive difference will disappear later. In every case, the live consumption-log total remains authoritative and the difference is rendered explicitly.
