...
Key 用量查询默认仅管理员可见，并采用服务端 POST 转发、固定上游路径、按用户和来源地址限流。原始 Key 不写入监控数据库、URL、配置审计或接口响应；最低角色只能设为运维员或管理员。

New API 功能页与传统 Key 用量查询是两条独立边界：
...
- New API Admin 与 Root 默认映射为监控管理员；普通账号默认只能访问四个个人 New API 功能页。监控总览、渠道、官方状态、日志、资源、事件、Key 查询和配置接口均在服务端要求运维员或管理员角色。
...
- 生产环境必须让 New API 功能页与 New API 使用同一浏览器 Origin；功能页、初始化向导和 Key 查询链路禁止携带凭据跟随重定向。
- 显式退出使用独立 HttpOnly 监控 Cookie 抑制自动 SSO；监控平台不得删除或修改 New API `session` Cookie，恢复 SSO 必须经过同源校验保护的显式 POST。
