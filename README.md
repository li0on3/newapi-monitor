...
![使用合成数据生成的 New API 功能页](docs/screenshots/customer-console-demo-zh.png)
...
- 默认采用低噪音的“体验严重告警”：最近 20 次可测请求中至少 15 次首字超过 15 秒才告警；单次总耗时再长也不会单独触发。
- 渠道仅在连续 5 次探测全部失败，或最近 10 次中至少 5 次失败时告警；其他资源、采集器和官方状态异常继续记录在事件页，但默认不外发。
...
- 整体视觉与 New API 默认前端保持一致，支持跟随系统、浅色和深色三种主题，并实时持久化切换结果。
- 页面动态配置，不写回 New API，不影响 New API 升级。
- 监控总览、概览、数据看板、API 密钥和真实使用日志作为普通用户的独立顶级模块展示；业务数据始终来自当前用户的 New API Session，不在监控平台复制客户业务库。
- 数据看板提供按模型堆叠的交互式趋势图，可切换请求数、Token 和额度，并查看峰值、时间桶平均值、模型排行和额度流向。
- API 密钥支持创建、编辑、启停、删除、批量删除和单次明文查看；明文 Key 不落库、不写日志、不进入审计和浏览器持久化存储。
- API 密钥支持监控平台独立多重分组，一个密钥可同时属于多个统计分组，并按 1/7/30 天查看个人密钥与分组真实用量；自定义密钥分组不改变 New API 的模型路由、计费或权限。
- 管理员可分别配置管理端和用户端的监控总览渠道清单；隐藏渠道只影响对应角色的卡片与状态汇总，不停止探测、日志采集或告警。
- 普通用户界面使用中性“API 服务中心”身份，不展示 New API 品牌和技术模块；管理员与运维员仍保留完整运维视图。
...
/monitor/system/console         New API 功能页配置
/monitor/console                概览
/monitor/console/analytics      数据看板
/monitor/console/keys           API 密钥
/monitor/console/logs           使用日志
```
...
- 数据库超过配置容量、出现告警死信，或待投递告警积压超过 15 分钟。

管理员可以在“系统配置 → 运行状态”查看每个采集器最后成功时间、连续失败次数、错误摘要和失效阈值。

总览也会校验渠道清单同步的新鲜度。同步失效时，旧快照仍保留用于排查，但不会继续被标记为实时健康数据；页面会将渠道状态置为未知并显示同步中断提示。
...
| 渠道真实探测 | 5 分钟 |
| OpenAI 官方状态 | 60 秒 |
| 慢请求展示 | 总耗时或首字超过 60 秒，仅用于页面统计 |
| 首字严重告警 | 最近 20 次可测请求中至少 15 次首字超过 15 秒 |
| 首字恢复 | 连续 10 次首字不超过 15 秒 |
| 渠道不可用 | 连续 5 次全部失败，或最近 10 次至少 5 次失败 |
| 渠道恢复 | 连续 5 次探测成功 |
| 其他异常 | 保留事件记录；体验严重模式下默认不外发 |
| 原始采样保留 | 90 天 |
| 已恢复事件保留 | 365 天 |
| 已投递/死信记录保留 | 30 天 |
| 数据库维护 | 每 6 小时 |
...
- 监控数据不匿名公开：普通 New API 用户通过现有 Session 实时识别，只能访问按用户端范围过滤的监控总览，以及个人概览、数据看板、API 密钥和使用日志；New API Admin 与 Root 自动成为监控管理员并可查看全部监控模块。
- New API 功能页必须通过 New API Session 登录。监控角色只决定是否显示入口，实际全局/个人数据范围继续由 New API 原始角色决定；紧急管理员不能访问这些业务页面。
- New API 功能页应与 New API 部署在同一浏览器 Origin 下（推荐挂载到 `/monitor/`），否则浏览器无法同时复用 New API Session 与 `uid`。
- New API 功能页只转发代码内固定的 New API API 白名单，不接受任意上游 URL、路径或请求头；普通用户单次查询最多 30 天。
- 业务数据按请求读取，不写入监控数据库。密钥明文只在用户主动查看的单次响应中出现，接口响应禁止缓存。
- 自定义密钥分组仅保存名称、颜色和按 `user_id + token_id + group_id` 隔离的成员关系；同一密钥可属于多个分组，账户总计按密钥去重，分组总计因重叠不可直接相加。
- 点击监控平台退出后，刷新不会自动复用残留 New API Session；用户必须在登录页主动选择“使用 New API 账号登录”。该操作只控制监控会话，不会退出或修改 New API。
- Key 用量查询默认仅管理员可用，最低只能下调到运维员；管理员可在“系统配置 → Key 用量查询”中启停功能、调整单次日志数量、查询频率和额度换算单位。
