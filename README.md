...
- 概览、数据看板、API 密钥和真实使用日志作为独立顶级模块展示；业务数据始终来自当前用户的 New API Session，不在监控平台复制客户业务库。
- API 密钥支持创建、编辑、启停、删除、批量删除和单次明文查看；明文 Key 不落库、不写日志、不进入审计和浏览器持久化存储。
- API 密钥支持监控平台独立分组，并按 1/7/30 天查看个人密钥与分组真实用量；自定义密钥分组不改变 New API 的模型路由、计费或权限。
- 管理员与运维员可配置监控总览的渠道清单；隐藏渠道只影响监控总览展示和状态汇总，不停止探测、日志采集或告警。
...
- 监控数据不匿名公开：普通 New API 用户通过现有 Session 实时识别，只能访问个人概览、数据看板、API 密钥和使用日志；New API Admin 与 Root 自动成为监控管理员并可查看全部监控模块。
